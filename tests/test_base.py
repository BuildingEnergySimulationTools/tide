...
class DumbProcessor(BaseProcessing):
    def __init__(self, required_features, keep_required):
        BaseProcessing.__init__(self, required_features)
    ...
    def _fit_implementation(self, X, y=None):
        ...
        if not self.keep_required:
            self.feature_names_out_ = [
                col for col in X.columns if col not in self.required_features
            ]
        self.feature_names_out_.append("new_col__°C__meteo")
        return self

    def _transform_implementation(self, X):
        X["new_col__°C__meteo"] = X[self.required_features] * 2
        if not self.keep_required:
            X.drop(self.required_features, axis=1, inplace=True)
        return X
...
class TestBase:
    def test_base_processing(self):
        ...
        dp = DumbProcessor(required_features=["text__°C__meteo"], keep_required=False)
        ...
        dp = DumbProcessor(required_features=["text__°C__meteo"], keep_required=True)
        dp.fit(data)
        assert list(dp.get_feature_names_out()) == [
            "text__°C__meteo",
            "hr__%hr__meteo",
